...
- **Sản phẩm (Production):** Vercel domain FE sẽ được set tương ứng.
...
- **Body:**
  ...
  ```
- FE cần lưu `data.access_token` làm Bearer token để gọi các API lấy OTP.
...
*(Các xử lí này đã có sẵn code mẫu ở file `fe/index.html` trong mã nguồn backend)*.
...
```
*-> FE học viên lưu `token` này làm thẻ bài truy cập. Token này có hiệu lực dài hạn, thường là 30 ngày.*
...
```
*-> FE khách hàng cần kiểm tra cứng `data.valid === true` chứ không phụ thuộc vào `success` của package chung. Sau đó show `data.message` ra toast error.*
